...
<body>
    ...
    $consulta1 = "SELECT * FROM peliculas ORDER BY RAND() LIMIT 5";
    ...
    echo "<h3>Peliculas mejor valoradas</h3>";
    $consulta3 = "SELECT p.*, AVG(c.calificacion) as PromedioCalificacion
                  FROM peliculas p
                  JOIN calificaciones c ON p.id = c.id_pelicula
                  GROUP BY p.id
                  ORDER BY PromedioCalificacion DESC
                  LIMIT 7";
    $result1 = $conn->query($consulta3);
